...
class Processor:
    ...
    def process(self, key):
        ...
        self.print_lectures_table_to_file(
            self.data[key],
            file_path=self.base_dir / f"specializations/{key}/output.pmwiki",
        )
    ...
    def print_lectures_table_to_file(self, url, file_path: Path):
        ...
        content_rows = table.find_element(By.CLASS_NAME, "tablecontent").find_elements(
            By.TAG_NAME, "tr"
        )
        ...
        with open(file_path, "w") as file:
            for row_idx in range(len(content_rows)):
                if row_idx in table_data["LV-Nr."]:
                    ...
                    logging.info(
                        f"Written to file: {table_data['LV-Nr.'][row_idx]} {title}"
                    )
    ...
    def get_specializations_links(self, url):
        ...
        try:
            ...
            iframe = WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located((By.ID, "audience"))
            )
            self.driver.switch_to.frame(iframe)
            section = WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(
                    (By.ID, "0xD7C07D509F5B453399672E5D323CB79B")
                )
            )
