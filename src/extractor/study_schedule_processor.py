...
class StudyPlanProcessor:
    ...
    def get_container(self, hierarchy_class, titel):
        if hierarchy_class == "hierarchy1":
            ...
            return self.study_schedule.setdefault(self.current_study, {}).setdefault(
                self.current_section, {}
            )
        elif hierarchy_class == "hierarchy4":
            self.current_brick = titel
            return (
                self.study_schedule.setdefault(self.current_study, {})
                .setdefault(self.current_section, {})
                .setdefault(self.current_module, {})
            )
        return None
    ...
    def process_rows(self, rows):
        for row in tqdm(rows, desc="Processing rows"):
            hierarchy_class = [
                cls for cls in row.get_attribute("class").split() if "hierarchy" in cls
            ][0]  # Get the hierarchy level from class attribute
