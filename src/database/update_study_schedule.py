...
module_brick_association = Table(
    "module_brick",
    Base.metadata,
...
# Association table for the many-to-many relationship
field_module_association = Table(
    "field_module",
    Base.metadata,
...
class Field(Base):
    ...
    modules = relationship(
        "Module", secondary=field_module_association, back_populates="fields"
    )
...
class Module(Base):
    ...
    link = Column(String)
    bricks = relationship(
        "Brick", secondary=module_brick_association, back_populates="modules"
    )
    fields = relationship(
        "Field", secondary=field_module_association, back_populates="modules"
    )
...
class Brick(Base):
    ...
    link = Column(String)
    modules = relationship(
        "Module", secondary=module_brick_association, back_populates="bricks"
    )
...
def update_study_schedule():
    ...
    uninteresting_parts = ["type", "CP", "link"]

    for field_name, field_content in study_schedule[
        "88-048-H-2018 – Elektrotechnik und Informationstechnik Master 2018"
    ].items():
